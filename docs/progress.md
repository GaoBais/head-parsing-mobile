...
- 2026-07-04: Extended the LiteRT Torch 0.9.x uint8 compatibility patch to cover the reverse `ui8` to `torch.uint8` JAX bridge mapping used while lowering the `to_channel_last_io` permute wrapper, and force the channel-last wrapper into eval mode before conversion. Local `.venv`: `python -m unittest discover -s tests` passed (74 tests, 1 skipped).
- 2026-07-04: Added a 256 fine-tune path for v3 acceptance failures: `scripts/train.py --init-checkpoint` now initializes model weights without restoring optimizer/scheduler state, `configs/train_mouth2teeth_9cls_finetune_256.yaml` provides a short low-LR schedule, and `docs/mobile_v3_export.md` documents the train/evaluate/re-export flow.
